...
class Go1MujocoEnv(MujocoEnv):
    ...
    def __init__(self, ctrl_type="torque", **kwargs):
        ...
        model_path = Path(f"./sim/unitree_go1/scene_{ctrl_type}.xml")
        MujocoEnv.__init__(
            self,
        ...
        # observation_dim: 45
        observation_dim = self._get_obs().shape
        self.observation_space = spaces.Box(
            low=-np.inf, high=np.inf, shape=observation_dim, dtype=np.float64
        )
    ...
    def _get_obs(self):
        ...
        # self.data.qpos: 19 dim
        # 0,1,2: global x,y,z position
        # 3,4,5,6: quaternion trunk
        # 7-18: 12 joint values
        ...
        # self.data.qpos: 18 dim
        # 0,1,2: global linear velocity of the robot
        # 3,4,5: angular velocity of the robot
        # 6-17: 12 joint values
        velocity = self.data.qvel.flatten()
        velocity[:3] *= self._velocity_scale
        # self._desired_velocity: currently just 1 0 0
        # self._velocity_scale: 2.0, 2.0, 0.25
    ...
    def _sample_desired_vel(self):
        ...
        return np.array([1.0, 0.0, 0.0])  # TODO: Train with randomized desired_vel
