...
def save_agent(agent, save_path, state_norm, reward_scaling):
    agent.actor.save(f'{save_path}/quadruped_actor')
    agent.critic.save(f'{save_path}/quadruped_critic')
    with open(f'{save_path}/quadruped_state_norm', 'wb') as file1:
        pickle.dump(state_norm, file1)
    with open(f'{save_path}/quadruped_reward_scaling', 'wb') as file2:
...
def main(args, number):
    ...
    model_save_path = f"./models/{train_time}"
